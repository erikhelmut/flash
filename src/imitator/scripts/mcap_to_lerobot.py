    ...
    try:
        ...
        for topic, msg, _ in rr.read_messages():
            ...
            if topic == "/nta_left":
                ...
                data["nta_left"].append(image_16bit)
                ...
                raw_data = np.frombuffer(msg.data, dtype=np.uint16)
                image_16bit = raw_data.reshape((msg.height, msg.width))
                data["nta_right"].append(image_16bit)
                data["nta_right_sum"].append(np.sum(image_16bit))
                data["nta_right_ts"].append(convert_timestamp_to_float(msg.header.stamp))
                if shapes["nta_right"] is None: 
                    shapes["nta_right"] = (msg.height, msg.width, 1)
...
def add_episode_to_dataset(dataset, synced_data, task_name, subsample=1):
    ...
    full_state_vector = np.concatenate([
        nta_left_sum, nta_right_sum, aruco_dist,
    ...
    for i in range(num_transitions):
        frame = {
            ...
            "observation.image.nta_left": img_nta_l_ep[i],
            "observation.image.nta_right": img_nta_r_ep[i],
    ...
    features = {
        **DEFAULT_FEATURES,
        "observation.state": {
            ...
            "names": ["nta_left", "nta_right", "aruco_dist", "ot_tx", "ot_ty", "ot_tz", 
        ...
        "observation.image.nta_left":  {"shape": (96, 96, 3), "dtype": "image", "names": ["height", "width", "channel"]},
        "observation.image.nta_right": {"shape": (96, 96, 3), "dtype": "image", "names": ["height", "width", "channel"]},
        "action": {
            "shape": (12,), "dtype": "float32",
            "names": ["nta_left", "nta_right", "aruco_dist", "ot_tx", "ot_ty", "ot_tz", 
                      "ot_rot_f0", "ot_rot_f1", "ot_rot_f2", "ot_rot_f3", "ot_rot_f4", "ot_rot_f5"]
